...
func deposit(w http.ResponseWriter, req *http.Request) {
	...
	if number, err := strconv.ParseFloat(numberqs, 64); err != nil {
		...
		if !ok {
			fmt.Fprintf(w, "Account with number cant be found")
		} else {
...
func sendmoney(w http.ResponseWriter, req *http.Request) {
	...
	if number, err := strconv.ParseFloat(numberqs, 64); err != nil {
		...
		if accountA, ok := accounts[number]; !ok {
			...
			err := accountA.SendMoney(amount, accountB)
			if err != nil {
				fmt.Fprintf(w, "%v", err)
			} else {
...
// http://localhost:8000/withdraw\?number=1001&amount=100
func main() {
